...
- p. 54, in paragraph "Comparisons, visual and numerical" $[80\% \pm 1.4\%]$ to $[74\%\pm 1.3\%]$ should be $[80\% \pm 1.3\%]$ to $[74\%\pm 1.4\%]$ (thanks to Ravi Shroff)
...
- p. 147, $W=\mbox{Diag}(w)$ should be $W=\mbox{Diag}(w^{-1})$ (thanks to Ravi Shroff)
...
- p. 243, "We have already seen that the coefficient for distance is 0.47" $\rightarrow$ "We have already seen that the coefficient for arsenic is 0.47" (thanks to Jordan Sucher)
- p. 248, Figure 14.5, the simulation results for $\beta_1$ (`dist100`) should be $-0.74$, $-0.51$, $-0.64$, $-0.67$ (the previous simulation results shown were for `dist`) (thanks to Clay Ford)
- p. 249   `z_new <- X_new %*% t(sims[s,]) + epsilon_new` $\rightarrow$   `z_new <- X_new %*% t(sims[s,, drop=FALSE]) + epsilon_new` (thanks to Clay Ford)
...
- p. 276, In the table comparing parameterizations, for Model 15.10 the second row $-c_{2|3}/\beta$ $\rightarrow$ $c_{2|3}/\beta$ (thanks to Gwebdolyn Eadie)
