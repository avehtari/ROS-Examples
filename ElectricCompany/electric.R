...
#' **Another plot**
#+ eval=FALSE, include=FALSE
if (savefigs) postscript(root("ElectricCompany/figs","electricscatter1a.ps"), horizontal=T, height=4)
#+
...
#' **Yet another plot**
#+ eval=FALSE, include=FALSE
if (savefigs) postscript(root("ElectricCompany/figs","electricscatter1b.ps"), horizontal=T, height=4)
#+
par(mfrow=c(1,4), pty="s")
for (j in 1:4){
  ok <- Grade==j
...
#' **Plot more**
#+ eval=FALSE, include=FALSE
if (savefigs) postscript(root("ElectricCompany/figs","electricscatter2.ps"), horizontal=T, height=4)
#+
par(mfrow=c(1,4), pty="s")
for (j in 1:4){
  ok <- Grade==j
...
fit_3 <- stan_glm(post_test ~ treatment + pre_test + treatment:pre_test,
                  subset=(grade==4), data=df, refresh=0)
print(fit_3)
...
#' **Plot linear model**
#+ eval=FALSE, include=FALSE
if (savefigs) postscript(root("ElectricCompany/figs","grade4.interactions.ps"), horizontal=T, height=3.8, width=5)
#+
n_sims <- nrow(sims_4)
...
for (k in 1:4) {
    ...
    fit_2 <- stan_glm(post_test ~ treatment + pre_test, subset=(grade==k),
                      data = df, refresh = 0, save_warmup = FALSE, 
                      open_progress = FALSE)
    est1[k] <- coef(fit_1)[2]
    est2[k] <- coef(fit_2)[2]
    se1[k] <- se(fit_1)[2]
...
#+ eval=FALSE, include=FALSE
if (savefigs) postscript(root("ElectricCompany/figs","electricsupp1.ps"), horizontal=T, height=2.6)
#+
...
for (k in 1:4){
  ...
  fit_supp <- stan_glm(supp ~ pre_test, family=binomial(link="logit"),
                       subset=ok, data=df, refresh = 0, 
                       save_warmup = FALSE, open_progress = FALSE)
  print(fit_supp)
  ...
  fit_supp <- stan_glm(post_test ~ supp + pre_test,
                       subset=((grade==k)&!is.na(supp)), data=df, refresh = 0, 
                       save_warmup = FALSE, open_progress = FALSE)
  print(fit_supp)
