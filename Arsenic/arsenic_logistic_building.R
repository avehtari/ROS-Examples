...
#' **Average predictive diffs for distance to nearest safe well**<br>
#' fit_7: y ~ dist100 + arsenic + educ4
b <- coef(fit_7)
hi <- 1
lo <- 0
delta <- invlogit (b[1] + b[2]*hi + b[3]*wells$arsenic + b[4]*wells$educ4) -
         invlogit (b[1] + b[2]*lo + b[3]*wells$arsenic + b[4]*wells$educ4)
print(mean(delta), digits=2)

#' **Average predictive diffs for arsenic level**<br>
#' fit_7: y ~ dist100 + arsenic + educ4
b <- coef(fit_7)
hi <- 1.0
lo <- 0.5
...
#' **Average predictive diffs for education**<br>
#' fit_7: y ~ dist100 + arsenic + educ4
b <- coef(fit_7)
hi <- 3
lo <- 0
...
#' **Example model with interaction**<br>
#' avg pred diffs for distance<br>
#' fit_8a <- y ~ c_dist100 + c_log_arsenic + c_educ4 +
#' c_dist100:c_educ4 + c_log_arsenic:c_educ4,
b <- coef(fit_8a)
hi <- 1
lo <- 0
delta <- invlogit(b[1] + b[2]*hi + b[3]*wells$c_log_arsenic +
                  b[4]*wells$c_educ4 + b[5]*hi*wells$c_educ4 +
                  b[6]*wells$c_log_arsenic*wells$c_educ4) -
         invlogit(b[1] + b[2]*lo + b[3]*wells$c_log_arsenic +
