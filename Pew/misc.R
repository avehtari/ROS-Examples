setwd("~/AndrewFiles/books/regression.and.other.stories/Examples/Pew")

## miscellaneous analyses using raw pew data

library("foreign")

pew_pre <- read.dta("pew_research_center_june_elect_wknd_data.dta")
n <- nrow(pew_pre)
table (pew_pre[,"date"])
...
pdf("pid.pdf", height=4.5, width=5.5)
...
pdf("ideology.pdf", height=4.5, width=5.5)
par(mar=c(3,2,2.5,1), mgp=c(1.5,.7,0), tck=-.01)
plot(c(1,n.inc), c(0,1), xaxs="i", yaxs="i", type="n", xlab="", ylab="", xaxt="n", yaxt="n")
axis(1, 1:n.inc, rep("",n.inc))
axis(1, seq(1.5,n.inc-.5,length=3), c("Low income", "Middle income", "High income"), tck=0)
...
for (i in 1:3){
  for (j in 1:3){
    plot(1:n.inc, prop[i,j,], type="l", bty="l", xaxs="i", yaxs="i", ylim=c(0,max(prop)), xaxt="n", yaxt="n", xlab="", ylab="")
    axis(1, 1:n.inc, rep("",n.inc))
...
mtext("Income distributions within self-reported political categories in 2008", outer=TRUE, side=3, line=.5, cex=1.2)
dev.off()
